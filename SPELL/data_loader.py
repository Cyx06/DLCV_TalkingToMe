...
class AVADataset(Dataset):
    ...
    def process(self):
        ...
        with open('csv_files/ttm_{}.csv'.format(self.mode)) as f:
        ##########################################################################################
            reader = csv.reader(f)
            data_gt = list(reader)
        ...
        for entity_id, frame_timestamp, x1, y1, x2, y2, video_id, label in data_gt:
        ##########################################################################################
            if video_id == 'video_id':
                continue
            vte = (video_id, float(frame_timestamp), entity_id)
            x1, y1, x2, y2 = float(x1)/1920.0, float(y1)/1080.0, float(x2)/1920.0, float(y2)/1080.0
        ...
        for ct, fl in enumerate(files):
            ...
            for i in tqdm(range(0, len_data, self.skip)):
                ...
                if self.edge_weight == 'fsimy':
                    data = Data(x=x, edge_index=edge_index, y=y, edge_attr=edge_attr, times=times, identity=identity)
                else:
                    data = Data(x=x, edge_index=edge_index, y=y, times=times, identity=identity)
                #######################################################################################################
